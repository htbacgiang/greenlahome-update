...
export default function FAQSection() {
  ...
  return (
    <section className="py-6 md:py-20 bg-q8-primary-50">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          {/* Left: Text + FAQ accordion */}
          <div>
            <div className="flex items-center gap-2 text-green-600 text-sm font-bold uppercase mb-2">
            ...
            <p className="text-gray-600 text-base leading-relaxed mb-8 max-w-xl">
              Đội ngũ GreenLa Home luôn lắng nghe và làm việc chặt chẽ để hiểu ý tưởng của bạn và hiện thực hóa với sự chăm chút từng chi tiết.
            </p>
              ...
              {faqItems.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    layout
                    className={`border rounded-xl overflow-hidden bg-white transition-all duration-300 ${isOpen ? 'border-green-200 shadow-md' : 'border-gray-100 hover:border-green-100'}`}
                  >
                    <button
                      type="button"
                    ...
                    >
                      <span className={`font-bold text-sm md:text-base pr-2 transition-colors duration-300 ${isOpen ? 'text-green-700' : 'text-gray-800'}`}>
                        <span className="text-green-600 font-semibold mr-2">{index + 1}.</span> {item.question}
                      </span>
                      <motion.span
                        className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-300 ${isOpen ? 'bg-green-600 text-white' : 'bg-gray-50 text-gray-500'}`}
                        aria-hidden
          ...
          >
            <Image
              ...
              alt="Không gian nội thất cao cấp - GreenLa Home"
              fill
